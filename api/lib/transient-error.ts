...
/**
 * Determine whether a GraphQL error means auto-merge is not allowed on the PR.
 *
 * GitHub returns `GraphqlResponseError` with `errors[].type === "UNPROCESSABLE"`
 * and a human-readable message containing "not allowed" when
 * `enablePullRequestAutoMerge` is called on a repository that lacks the
 * required branch protection rules. The type identifier
 ...
 * fails — the same structural bug that affected `isAutoMergeNotEnabledError`.
 *
 * Uses duck-typing on `.name` for the same cross-version `@octokit/graphql`
 * reason documented on `isAutoMergeNotEnabledError`.
 */
